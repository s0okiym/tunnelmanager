...
| `tunnel -L [bind:]port:host:hostport --udp` | 本地 UDP 端口转发 |
...
| `tunnel stop` | 停止守护进程 |
| `tunnel ls` | 列出所有隧道 |
| `tunnel status [name]` | 查看隧道状态/守护进程摘要 |
| `tunnel logs [name] [--lines N]` | 查看隧道/全局日志 |
| `tunnel add -L/-R/-D ... [--name X] [--group G] [--autostart=false]` | 添加隧道 |
| `tunnel rm <name>` | 删除隧道 |
| `tunnel reload` | 热重载配置 |
| `tunnel enable <name>` | 启用并启动隧道 |
| `tunnel disable <name>` | 禁用并停止隧道 |
| `tunnel restart <name>` | 重启隧道 |
| `tunnel save` | 保存运行时配置 |
...
|------|------|
| `--tls` | 启用 TLS 1.3 加密 |
| `--server-fingerprint SHA256:...` | 固定信任远端 TLS 身份指纹 |
| `--trust-on-first-use` | 首次连接时自动记录并信任远端指纹 |
| `--token <str>` | 设置认证令牌 |
| `--udp` | UDP 模式 |
...
当 `--tls` 不搭配 `--tls-cert`/`--tls-key` 时，tunnel 会自动生成并持久化一个自签名 TLS 身份到 `~/.config/tunnel/identity/`。身份密钥（`cert-key.pem`）长期不变，证书（`cert.pem`）会在到期前 30 天自动轮换，因此服务端身份指纹（基于公钥）是稳定的，可用于类似 SSH `known_hosts` 的验证。

服务端启动时会输出身份指纹：

```bash
tunnel -s 0.0.0.0:9000 --tls
# 日志输出：Remote server TLS identity fingerprint: SHA256:...
```

客户端可以通过以下两种方式验证服务端身份，无需 CA：

```bash
# 方式一：直接指定信任的服务端指纹
tunnel -R 9090:localhost:8080 -s server.example.com:9000 --tls \
  --server-fingerprint SHA256:ABC123...

# 方式二：首次连接时自动记录指纹（TOFU）
tunnel -R 9090:localhost:8080 -s server.example.com:9000 --tls \
  --trust-on-first-use
```

`--trust-on-first-use` 会把指纹写入 `~/.config/tunnel/known_hosts`。如果后续连接发现指纹变化，会拒绝连接并提示可能的中间人攻击。
...
tunnels:
  - name: remote-client
    ...
    tls: true
    tls_server_fingerprint: "SHA256:ABC123..."
    # 或者
    # tls_trust_on_first_use: true
    autostart: true
```
...
- 默认自动生成 ECDSA P-256 自签名证书，有效期 1 年，到期前 30 天自动轮换
- 自动身份的指纹基于长期密钥，证书轮换后 `--server-fingerprint` 与 `known_hosts` 仍然有效
...
# 启用已存在的隧道（设置 autostart 并启动）
tunnel enable web-tunnel

# 禁用隧道（停止并取消 autostart）
tunnel disable web-tunnel

# 重启隧道
tunnel restart web-tunnel
...
# 单个隧道详细状态
tunnel status mysql-tunnel

# 全局最近日志（默认 100 行）
tunnel logs

# 单个隧道最近日志
tunnel logs mysql-tunnel

# 指定行数
tunnel logs mysql-tunnel --lines 50
```
...
| `token` | string | 认证令牌 |
| `tls` | bool | 是否启用 TLS |
| `tls_cert` | string | TLS 证书文件路径（PEM） |
| `tls_key` | string | TLS 私钥文件路径（PEM） |
| `tls_verify` | bool | 是否验证对端 TLS 证书（需受信 CA） |
| `tls_trust_on_first_use` | bool | 首次连接时自动记录服务端指纹 |
| `tls_server_fingerprint` | string | pinned 的服务端证书指纹（`SHA256:...`） |
...
A: 建议同时使用 `--tls`（TLS 1.3 加密）和 `--token`（令牌认证）。在公网 remote 转发场景中，可额外使用 `--server-fingerprint` 或 `--trust-on-first-use` 固定/校验服务端身份，防止中间人攻击。不要在不安全的网络上使用明文模式。
