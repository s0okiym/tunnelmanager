...
Guidance for AI coding agents working on this repository. Everything below is
grounded in the actual source, not the roadmap. Where `DESIGN.md` / `README.md`
describe features that are **not** implemented, this file flags them explicitly
so you don't trust stale prose — treat this file as the source of truth for
what the code really does.

## Project overview

`tunnel` is a single-binary, port-forwarding tool written in Go, roughly an
`ssh -L/-R/-D` replacement with its own lightweight framed control protocol.
Three forwarding modes:

- **local** (`-L [bind:]port:host:hostport`) — listen locally, dial a target, relay bytes. Like `ssh -L`.
- **remote** (`-R port:host:hostport -s server:port` + a server side `-s addr`) — NAT penetration: the client dials out to a public server over a persistent control connection; the server accepts public connections and multiplexes them back over that control connection to the client's local target. Like `ssh -R`.
- **dynamic** (`-D port`) — a SOCKS5 (CONNECT only) proxy. Like `ssh -D`.

It runs in two ways: **ad-hoc** (foreground, zero config, driven by `-L/-R/-D/-s`
flags) and **daemon** (`tunnel start`, managed via a YAML config + a Unix-socket
control channel). No external network dependencies; only the Go stdlib plus
...
All of the above currently pass clean: `go build ./...` and `go vet ./...`
produce no output, and `go test -race -count=1 ./...` reports `ok` for all four
packages (`tunnel` root, `tunnel/e2e`, `tunnel/manager`, `tunnel/relay`).
Always run tests with `-race` — the relay and manager code is heavily concurrent
and the race detector is the primary safety net. The `e2e` package includes a
process-level test (`TestE2EDaemonLifecycle`) that `go build`s the binary and
drives the real daemon over its control socket, so the first `e2e` run compiles
the CLI.
...
│   │                         add/remove/list/groups/Reload) + HandleControl dispatch
...
│   ├── manager_test.go     — config round-trip, add/remove/list, HandleControl, pidfile
...
│   │                         remote listeners tied to the control-conn lifetime
...
│   └── daemon_test.go      — process-level: build binary, start/ls/forward/SIGHUP-reload/stop
...
`net.Listen` → per-connection goroutine → `net.Dial` the target → `Relay`.
`NewTLSProxy` wraps the listener with `tls.Listen`; the encrypted segment is
only client↔tunnel, the tunnel→target hop is plain TCP.

### Dynamic mode (`SocksProxy`, `socks5proxy.go` + `socks5.go`)
`net.Listen` → SOCKS5 greeting (no-auth only) → CONNECT request → parse target
(IPv4/IPv6/domain) → `net.Dial` → reply with bound address → `Relay`. Only the
CONNECT command is supported.
...
- **Reconnect:** `RemoteClient.Run` loops forever, using `BackoffDelay` (base 1s, cap 30s, 20% jitter, ≤30 doublings, infinite attempts). On the **server** side, each per-tunnel public listener's lifetime is tied to its control connection (see key gotchas), so a client reconnecting re-binds the same remote port cleanly.

### Daemon mode
`tunnel start` writes a pidfile, builds a `Manager`, autostarts tunnels with
`autostart: true`, and serves a Unix-socket control channel. Each managed tunnel
runs in its own goroutine cancelled via a per-tunnel `close(ctx)`; `stopLocked`
cancels every tunnel and then **waits for each goroutine to fully return**
(releasing its listener) before proceeding, so reloads don't race the old
listener's close. `SIGHUP` calls `Manager.Reload()` **in place** (same `*Manager`,
under `m.mu`) so the bound control-server handler keeps working after a reload.
`SIGINT`/`SIGTERM` and a `stop` control request both drive a graceful shutdown
of the whole daemon (via a shutdown channel; `stop` defers the shutdown briefly
so its response is flushed to the client first). CLI subcommands (`ls`, `add`,
`rm`, `reload`, `start-group`, `stop-group`, `stop`) are thin clients that dial
the socket and send one JSON request.
...
**Implemented:** local/remote/dynamic forwarding; TLS 1.3 (`--tls`, auto self-signed
or `--tls-cert`/`--tls-key`, `--tls-verify`); token auth; binary frame protocol +
multiplexer with per-connection channel cleanup; half-close; auto-reconnect with
backoff+jitter (server frees and re-binds the remote port across reconnects);
dead-peer detection on the control connection; daemon (`start [--background]`,
`stop` — which actually terminates the daemon, `ls`, `add`, `rm`, `reload`,
`start-group`, `stop-group`); in-place `SIGHUP` reload; signal handling;
`init --systemd` / `--systemd-user`; health check (log-only); `$TUNNEL_CONFIG`
env; `log_file`/`control_socket` global config; control socket restricted to
`0600` with an idle read timeout; `tunnel add` for all of `-L`/`-R`/`-D`
(`-R` requires `-s` and also captures `--token`/`--tls`/`--tls-verify`/
`--tls-cert`/`--tls-key`); config-file multi-hop and multi-connection fields.

**Present as a flag/field but NOT functional:**
- `--udp` — `main.go` explicitly `log.Fatal("UDP forwarding not yet implemented")`.
- `metrics` / `expvar` / Prometheus, and `tunnel logs`, `status`, `enable`, `disable`, `restart`, `save`, `--project` — described in docs but **not** wired into `main.go`'s subcommand switch.
- **Identity fingerprint / known_hosts / TOFU / cert persistence & rotation** — described at length in `DESIGN.md` §6, but not implemented. `GenerateCert` produces a fresh in-memory ECDSA P-256 cert (valid 1 year, SAN `127.0.0.1`) each run; nothing is written to `~/.config/tunnel/identity/`.
...
- **`connections: N`** — `runRemoteClientMulti` starts N independent `RemoteClient`s all registering the *same* remote port; the server does a plain `net.Listen` per registration, so the 2nd+ listener fails with address-in-use. True load balancing is not achieved; treat `connections > 1` as unsupported.
- `tunnel ls` status is only `running`/`stopped` (goroutine registered or not), not the rich `LISTENING`/`ESTABLISHED`/reconnect-count states shown in the docs. A tunnel whose goroutine failed asynchronously (e.g. bad address) can still read as `running`.
- The multiplexer has **no per-channel flow control**: `channel.rb` buffers inbound data unboundedly, and a stalled channel write blocks all writes on that control connection (head-of-line blocking). Fine for typical use; not designed for adversarial slow-consumer workloads.
...
- TLS tests generate ephemeral in-memory certs; don't rely on any on-disk cert state.
...
- **No mTLS / PKI, no fingerprint pinning** are implemented despite the design doc — the only peer-identity guarantee is `--tls-verify` against a CA-signed cert, plus the shared token.
...
- **Per-tunnel listener lifetime is tied to the control connection:** `serveRemoteTunnel` closes its public listener when `cc.Done()` fires, and `RemoteServer.Close()` tears down all tracked control connections (so `Wait()` returns and reconnects reuse the port). `listenTunnel` retries the bind briefly to cover the reconnect handoff. Don't reintroduce a listener that outlives its control connection.
...
- **`remoteserver.go` binds `0.0.0.0:<port>`** for each registered tunnel — hardcoded, ignores any bind address. Two *different* control connections registering the same port still collide.
